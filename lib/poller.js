...
class Poller {
  ...
  /**
   * Get convention-based flag name for a task
   * UI-001 -> ui001_done, SUP-050 -> sup050_done
   */
  getConventionFlagName(taskId) {
    if (taskId.startsWith('UI-')) {
      return 'ui' + taskId.replace('UI-', '').toLowerCase() + '_done';
    } else if (taskId.startsWith('SUP-')) {
      return 'sup' + taskId.replace('SUP-', '').toLowerCase() + '_done';
    }
    return null;
  }
  ...
  async checkTaskAcceptance() {
    ...
    for (const task of tasksToCheck) {
      try {
        ...
        const history = this.history.get(sourceName);
        if (!history || history.length === 0) {
          continue;
        }
        ...
        // Check convention-based flag first (ui001_done, sup001_done)
        const conventionFlag = this.getConventionFlagName(task.id);
        if (conventionFlag && latest.data[conventionFlag] === true) {
          console.log(`  ✓ ${task.id}: Convention flag "${conventionFlag}" is true! Marking as done.`);
          await tasksManager.updateTaskStatus(task.id, 'done');
          continue;
        }

        // Check acceptanceKey (custom field)
        if (task.acceptanceKey && latest.data[task.acceptanceKey] === true) {
          console.log(`  ✓ ${task.id}: Acceptance key "${task.acceptanceKey}" is true! Marking as done.`);
          await tasksManager.updateTaskStatus(task.id, 'done');
          continue;
        }

        // Fall back to fileContains check
        if (!task.acceptanceCheck || task.acceptanceCheck.type !== 'fileContains') {
          continue;
        }
