  ...
  async checkTaskAcceptance() {
    ...
    for (const task of tasksToCheck) {
      try {
        ...
        // Check acceptanceKey first (status JSON field)
        if (task.acceptanceKey) {
          const history = this.history.get(sourceName);
          if (history && history.length > 0) {
            const latest = history[history.length - 1];
            if (latest.success && latest.data && latest.data[task.acceptanceKey] === true) {
        ...
        // Fall back to fileContains check if acceptanceKey not met
