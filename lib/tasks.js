...
class TasksManager {
  ...
  /**
   * Get evaluator tasks sorted by dependencies
   * Returns: Task[] for evaluator roadmap
   */
  getEvaluatorTasks() {
    const evaluatorTasks = this.tasks.filter(task => task.app === 'evaluator');
    return this.topologicallySortedTasks(evaluatorTasks);
  }
}
