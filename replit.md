...
*   **ASK_LEAD_QUESTION Overconfidence Guard (Feb 2026):** When `step_type=ASK_LEAD_QUESTION`, Tower checks `metrics.confidence` and `metrics.evidence_items`. If confidence === 1.0 → STOP with `invalid_confidence`. If confidence > 0.85 → requires 2+ independent evidence sources (unique `source` fields) and at least one `is_official: true`; otherwise issues CHANGE_PLAN with action `retry` and reason `overconfident_without_support`. Confidence ≤ 0.85 passes without evidence checks. Implementation: `judgeAskLeadQuestion` in `src/evaluator/towerVerdict.ts`, route wiring in `server/routes-judge-artefact.ts`, tests in `tests/towerVerdict.test.ts`.
