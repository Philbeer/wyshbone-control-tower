...
*   **Tower Verdict (Agent Loop Judgement):** Deterministically evaluates leads list artifacts against user goals and constraints, returning structured verdicts (ACCEPT/CHANGE_PLAN/STOP/ASK_USER) with detailed rationales and suggested changes. Supports hard/soft constraint classification: hard constraints are never auto-relaxed (triggers ASK_USER verdict with user options when only hard constraints remain), while soft constraints may be relaxed in priority order (EXPAND_AREA location → BROADEN_QUERY business_type → RELAX_CONSTRAINT prefix_filter). Backwards compatible with legacy payloads that omit hard/soft arrays.
