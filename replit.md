...
Tower enforces time-based constraint honesty as a post-check in `judgeLeadsList`. Input fields: `time_predicates` (array of `{ predicate, hardness }`), `time_predicates_mode` (`"verifiable" | "proxy" | "unverifiable"`), `time_predicates_proxy_used` (nullable enum: `"news_mention" | "recent_reviews" | "new_listing" | "social_media_post" | "press_release"`), `time_predicates_satisfied_count`, `time_predicates_unknown_count`. Auto-detection via `detectTimePredicate()` scans goal text for phrases like "opened in last N months", "recently opened", "newly opened", "launched in last N", "established after/since". Auto-detected predicates default to hard. Verdict rules: (1) If any hard time_predicate is blocked (unverifiable with no proxy, or proxy mode with null proxy, or 0 satisfied), verdict overrides ACCEPT→STOP with code `TIME_PREDICATE_BLOCKED`. (2) If proxy used with evidence but `delivery_summary` doesn't say "proxy used" or "PARTIAL", verdict overrides to STOP with code `TIME_PREDICATE_PROXY_LANGUAGE_MISMATCH`. (3) Soft predicates do not block ACCEPT. Output includes `user_summary` one-liner for UI display. Stop codes: `TIME_PREDICATE_BLOCKED`, `TIME_PREDICATE_PROXY_LANGUAGE_MISMATCH`. Tests: 20 tests in `tests/towerVerdict.test.ts`.
