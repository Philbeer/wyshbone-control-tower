...
*   **ASK_LEAD_QUESTION Overconfidence Guard (Feb 2026):** When `step_type=ASK_LEAD_QUESTION`, Tower checks `metrics.confidence` and `metrics.evidence_items`. If confidence === 1.0 → STOP with `invalid_confidence`. If confidence > 0.85 → "verified" requires official site evidence (`is_official: true`) OR 2+ independent domains corroborating the claim; otherwise issues CHANGE_PLAN with action `retry`, reason `overconfident_without_support`, and specific `suggested_changes` (e.g., ADD_VERIFICATION_STEP) telling Supervisor what to do. Confidence ≤ 0.85 passes without evidence checks. **Template B attribute verification:** HARD attribute with `capability_says_unverifiable` or `evidence_sufficient=false` → STOP with `UNVERIFIABLE_HARD_CONSTRAINT`; SOFT attribute unverifiable → ACCEPT with `reason_flags` and `disclosure` in `metrics`. All responses include standardized fields: `towerVerdict`, `action`, `stop_reason` (with optional `detail`), `suggested_changes[]`, `metrics{}`. Implementation: `judgeAskLeadQuestion` in `src/evaluator/towerVerdict.ts`, route wiring in `server/routes-judge-artefact.ts`, tests in `tests/towerVerdict.test.ts`.
