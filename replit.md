...
*   **Artefact Judgement API:** Tower judges artifacts stored in Supabase by inspecting their `payload_json` and applying specific rules based on `artefactType` (e.g., `leads_list` artefacts are judged by count and constraint adherence, `factory_state`/`factory_decision` artefacts by plastics rubric) to determine whether to continue or stop a run.
